...
parser = ArgumentParser(description="Convert MLQA dataset to JSON format with optional tagging.")
parser.add_argument("--en_source_path", type=str, default="./MLQA/MLQA_V1/test/test-context-en-question-en.json", help="Path to English source file")
parser.add_argument("--test_folder_path", type=str, default="./MLQA/MLQA_V1/test", help="Path to test folder containing language files")
parser.add_argument("--max_length", type=int, default=482, help="Maximum length for untagged context (512 - 30)")
...
parser.add_argument("--tag_type", type=str, default="xml", choices=["squarebracket", "xml", None], help="Type of tags to use")
parser.add_argument("--languages", type=str, default="en,de,ar,es,vi,hi,zh", help="Comma-separated list of languages to process")
parser.add_argument("--do_scoring", type=bool, default=True, help="Whether to score contexts with COMET")
parser.add_argument("--scoring_threshold", type=float, default=0.75, help="Threshold for COMET scores")
...
OUT_PATH = Path(f"{args.out_dir}/MLQA{OUT_DIR_SUFFIX}")
...
for testfile in TEST_FOLDER_PATH.glob("*.json"):
    _, _, src_lang, _, tgt_lang = testfile.stem.split("-")
